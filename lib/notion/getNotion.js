...
export async function getNotion(pageId) {
  ...
  return {
    ...
    createdTime: { start_date: formatDate(new Date(postInfo?.last_edited_time).toString(), BLOG.LANG) },
    blockMap
  }
}
...
function getPageCover(postInfo) {
  ...
  if (pageCover) {
    ...
    if (pageCover.startsWith('https')) return defaultMapImageUrl(pageCover, postInfo)
  }
}
